...
#define TOUCH_THRESHOLD_BASE 25
#define RELEASE_THRESHOLD_BASE 20
...
void mpr121_init(uint8_t i2c_addr)
{
    ...
    write_reg(i2c_addr, 0x35, 20); // Delay limit Touched 
    ...
    const uint8_t usl = 201; //(3.3 - 0.0) / 3.3 * 256;
    write_reg(i2c_addr, 0x7D, usl),  
    write_reg(i2c_addr, 0x7E, usl * 0.2),
